...
export function MenuGrid({ category = 'All', searchQuery = '' }: MenuGridProps) {
  ...
  // Fetch ALL available items once on mount — category switching is done in memory,
  // so tab changes are instant with zero network requests.
  useEffect(() => {
    let cancelled = false
    ...
    async function fetchAllItems() {
      ...
      try {
        ...
        if (res.ok && !cancelled) {
          ...
          setAllItems(data.filter((item) => item.isAvailable))
        }
      } catch (err) {
  ...
  // Filter in memory — O(n) over a small dataset, instant UX
    ...
    return (
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-20 md:pb-0">
        {[...Array(8)].map((_, i) => (
          <div
            key={i}
            className="bg-white border border-border rounded-xl p-3 animate-pulse flex flex-col items-center"
          >
